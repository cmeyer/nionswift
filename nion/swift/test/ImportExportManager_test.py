...
class TestImportExportManagerClass(unittest.TestCase):
    ...
    def test_export_nhdf1_handles_composite_line_plot_uuids(self):
        with TestContext.create_memory_context() as test_context:
            ...
            current_working_directory = os.getcwd()
            file_path = os.path.join(current_working_directory, "__file.nhdf1")
            handler = ImportExportManager.HDF5ImportExportHandler("nhdf1-io-handler", "", ["nhdf1"])
            handler.write_display_item(display_item1, pathlib.Path(file_path), "nhdf1")
    ...
    def test_reloading_imported_data(self):
        with TestContext.create_memory_context() as test_context:
            ...
            with contextlib.closing(document_controller):
                ...
                file_path = os.path.join(current_working_directory, "__file.nhdf1")
                handler = ImportExportManager.HDF5ImportExportHandler("nhdf1-io-handler", "", ["nhdf1"])
                handler.write_display_item(display_item, pathlib.Path(file_path), "nhdf1")
                document_controller.receive_files([file_path])
                self.assertEqual(2, len(document_model.data_items))
                self.assertEqual(2, len(document_model.display_items))
            # reload and verify
