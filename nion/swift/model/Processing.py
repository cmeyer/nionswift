...
class ProcessingComputation:
    ...
    def __get_input_navigation(self, **kwargs: typing.Any) -> tuple[DataAndMetadata.ShapeType | None, Exception | None]:
        ...
        for source_d in self.processing_component.sources:
            ...
            if xdata:
                ...
                if navigation_dimension_shape != xdata.navigation_dimension_shape:
                    return None, ValueError("Mismatched navigation dimension shapes between sources.")
                # check whether all items have the same datum dimension shapes
                if datum_dimension_shape != xdata.datum_dimension_shape:
                    return None, ValueError("Mismatched datum dimension shapes between sources.")
                ...
                for i, cal in enumerate(xdata.navigation_dimensional_calibrations):
                    if navigation_calibrations[i] != cal:
                        return None, ValueError("Mismatched navigation dimension calibrations between sources.")
                ...
                for i, cal in enumerate(xdata.datum_dimensional_calibrations):
                    if datum_calibrations[i] != cal:
                        return None, ValueError("Mismatched datum dimension calibrations between sources.")
        if navigation_dimension_shape is None or datum_dimension_shape is None or navigation_calibrations is None or datum_calibrations is None:
            return None, ValueError("Could not determine navigation and datum dimension shapes and calibrations from sources.")
        return navigation_dimension_shape, None
    ...
    def execute(self, **kwargs: typing.Any) -> None:
        ...
        navigation_dimension_shape, error = self.__get_input_navigation(**kwargs)
        if error:
            raise error
